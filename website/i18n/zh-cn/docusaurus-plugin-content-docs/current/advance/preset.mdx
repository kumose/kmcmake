...
```
内置预设保持极简、可替换：
- `default`：`Unix Makefiles` → `build/`（Linux/macOS 基线）。
- `ninja`：`Ninja` → `build-ninja/`（更快；Windows MSVC CI 优先使用）。
- 共用 `base`，将 `CMAKE_TOOLCHAIN_FILE` 指向 `$env{KMPKG_CMAKE}`。**kmpkg 可选**：未设置环境变量即为空工具链；使用 kmpkg 时再设置即可。
...
- 不强制依赖 kmpkg/vcpkg：可将 `CMAKE_TOOLCHAIN_FILE` 换成任意工具链，或保持 `KMPKG_CMAKE` 为空。
- 不锁定生成器：直接使用内置 `default` / `ninja`，也可自行增加 `Visual Studio 17 2022` / `Xcode`。
